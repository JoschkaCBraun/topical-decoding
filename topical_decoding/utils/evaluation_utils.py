...
def get_topic_tokens(
    ...
    for word in topic_words:
        ...
        token_ids = [
            token_id for token_id in token_ids if token_id not in special_token_ids
        ]
...
def calculate_scores(experiment_results: dict, score_type: str, method: str) -> dict:
    ...
    if score_type == "topic" and method not in [
        "dict",
        "tokenize",
        "lemmatize",
        "stem",
        ...
        raise ValueError(
            f"Invalid method '{method}' for score_type 'topic'. Choose either 'dict', 'tokenize', 'lemmatize', or 'stem'."
        )
    if score_type == "quality" and method not in [
        "avg_rouge",
        "rouge1",
        "rouge2",
        "rougeL",
        ...
        raise ValueError(
            f"Invalid method '{method}' for score_type 'quality'. Choose either 'avg_rouge', 'rouge1', 'rouge2', 'rougeL', 'mauve', or 'bert'."
        )
    experiment_name = experiment_results["experiment_information"]["EXPERIMENT_CONFIG"][
        "experiment_name"
    ]

    if experiment_name == "baseline":
        if score_type == "topic":
            scores = baseline_topic_scores(
                experiment_results=experiment_results, method=method
            )
        elif score_type == "quality":
            scores = baseline_quality_scores(
                experiment_results=experiment_results, method=method
            )

    elif experiment_name in ["factor_scaling", "constant_shift", "threshold_selection"]:
        if experiment_name == "constant_shift":
            factors = experiment_results["experiment_information"][
                "CONSTANT_SHIFT_DICT"
            ]["shift_constants"]
        ...
        if score_type == "topic":
            scores = logits_reweighting_topic_scores(
                experiment_results=experiment_results, method=method, factors=factors
            )
        if score_type == "quality":
            scores = logits_reweighting_quality_scores(
                experiment_results=experiment_results, method=method, factors=factors
            )
    else:
        raise ValueError(
            "Invalid experiment name. Choose between 'baseline', 'factor_scaling',"
...
def baseline_topic_scores(experiment_results: dict, method: str) -> Dict[str, float]:
    ...
    scores = dict()
    lda = load_lda()
    focus_types = experiment_results["experiment_information"]["BASELINE_DICT"][
        "focus_types"
    ]

    for focus in focus_types:
        summaries = [
            summary[focus] for summary in experiment_results["generated_summaries"]
        ]
        tids_tid1 = [
            summary["tid1"] for summary in experiment_results["generated_summaries"]
        ]
        tids_tid2 = [
            summary["tid2"] for summary in experiment_results["generated_summaries"]
        ]

        scores_tid1 = calculate_topic_scores(
        ...
        scores_tid2 = calculate_topic_scores(
            experiment_results=experiment_results,
            lda=lda,
            texts=summaries,
            tids=tids_tid2,
...
def baseline_quality_scores(
    experiment_results: dict, method: str
    ...
    """
    dataset_name = experiment_results["experiment_information"]["DATASET_CONFIG"][
        "dataset_name"
    ]
    ...
    if method == "mauve":
        ...
        raise ValueError(
            "Invalid method. Choose 'avg_rouge', 'rouge1', 'rouge2', 'rougeL', 'mauve', or 'bert'."
        )
    ...
    for focus in focus_types:
        ...
        reference_summaries1 = [
            reference_summaries.iloc[idx]["summary1"]
            for idx in range(len(generated_summaries))
        ]
        reference_summaries2 = [
            reference_summaries.iloc[idx]["summary2"]
            for idx in range(len(generated_summaries))
        ]

        # Compute ROUGE scores for all summaries against both reference summaries
        if method in ["avg_rouge", "rouge1", "rouge2", "rougeL"]:
            quality_scores_1 = calculate_rouge_scores(
                text_predictions=generated_summaries,
                text_targets=reference_summaries1,
                method=method,
                scorer=scorer,
            )
            quality_scores_2 = calculate_rouge_scores(
                text_predictions=generated_summaries,
            ...
            quality_scores_1 = calculate_mauve_scores(
                text_predictions=generated_summaries,
                text_targets=reference_summaries1,
                mauve=mauve,
            )
            quality_scores_2 = calculate_mauve_scores(
                text_predictions=generated_summaries,
                text_targets=reference_summaries2,
                mauve=mauve,
            )
        elif method == "bert":
            quality_scores_1 = calculate_bert_scores(
                text_predictions=generated_summaries,
                text_targets=reference_summaries1,
                bert=bert,
            )
            quality_scores_2 = calculate_bert_scores(
                text_predictions=generated_summaries,
                text_targets=reference_summaries2,
                bert=bert,
            )
        ...
        quality_scores[focus] = {
            "summary1": quality_score_1,
            "summary2": quality_score_2,
        }

    return quality_scores


def logits_reweighting_topic_scores(
    experiment_results: dict, method: str, factors: list
) -> Dict[str, float]:
    """
    ...
    for factor in factors:
        summaries = [
            summary[str(factor)]
            for summary in experiment_results["generated_summaries"]
        ]
        tids = [
            summary["tid1"] for summary in experiment_results["generated_summaries"]
        ]
        scores_factor = calculate_topic_scores(
            experiment_results=experiment_results,
            lda=lda,
            texts=summaries,
            tids=tids,
...
def logits_reweighting_quality_scores(
    experiment_results: dict, factors: list, method: str
) -> Dict[str, float]:
    """
    ...
    for factor in factors:
        ...
        reference_summaries = [
            reference_texts.iloc[idx]["summary1"]
            for idx in range(len(generated_summaries))
        ]

        if method in ["avg_rouge", "rouge1", "rouge2", "rougeL"]:
            factor_scores = calculate_rouge_scores(
                text_predictions=generated_summaries,
            ...
            factor_scores = calculate_mauve_scores(
                text_predictions=generated_summaries,
                text_targets=reference_summaries,
                mauve=mauve,
            )
        elif method == "bert":
            factor_scores = calculate_bert_scores(
                text_predictions=generated_summaries,
                text_targets=reference_summaries,
                bert=bert,
            )
...
def calculate_topic_scores(
    ...
    num_topic_words = experiment_results["experiment_information"]["TOPICS_CONFIG"][
        "num_topic_words"
    ]
    ...
    if method == "dict":
        ...
        model_alias = experiment_results["experiment_information"]["EXPERIMENT_CONFIG"][
            "model_alias"
        ]
        return calculate_tokenizer_topic_scores(
            texts=texts,
            lda=lda,
            tids=tids,
        ...
        return calculate_stem_or_lem_topic_scores(
            texts=texts,
            lda=lda,
            tids=tids,
            method=method,
        ...
        raise ValueError(
            "Invalid method. Choose 'dict', 'tokenize', 'lemmatize', or 'stem'."
        )
...
def calculate_mauve_scores(
    text_predictions: List[str], text_targets: List[str], mauve: Any
) -> List[float]:
    """
...
def calculate_bert_scores(
    text_predictions: List[str], text_targets: List[str], bert: BERTScorer
) -> List[float]:
    """
...
def calculate_dict_topic_scores(
    texts: List[str], tids: List[int], lda: LdaModel
) -> List[float]:
    """
    ...
    return [
        prevalences.get(tid, 0.0) for prevalences, tid in zip(document_topics, tids)
    ]


def calculate_tokenizer_topic_scores(
    texts: List[str],
    lda: LdaModel,
    tids: List[int],
    num_topic_words: int,
    ...
    for text, tid in zip(texts, tids):
        topic_tokens = set(
            get_topic_tokens(
                tokenizer=tokenizer, lda=lda, tid=tid, num_topic_words=num_topic_words
            )
        )
...
def calculate_stem_or_lem_topic_scores(
    texts: List[str], lda: LdaModel, tids: List[int], num_topic_words: int, method: str
) -> List[float]:
    """
    ...
    def process_text(text: str, method: str):
        ...
        if method == "lemmatize":
            ...
            processed_words = {
                ...
                if not (token.is_punct or token.is_space or token.text in stopwords)
            }
        elif method == "stem":
            words = text.split()
            processed_words = {
...
def doc_topics(
    document: str, lda: LdaModel, dictionary: Dictionary
) -> Dict[int, float]:
    """
...
def ab_topic_diff_score(
    tid_a: int, tid_b: int, document: str, lda: LdaModel, dictionary: Dictionary
) -> float:
    """
