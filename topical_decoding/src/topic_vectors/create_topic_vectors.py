...
from utils.read_and_load_utils import load_model_and_tokenizer, read_dataset, load_lda, get_topic_vectors_folder_name
from utils.evaluation_utils import get_topic_words
from steering_vectors import train_steering_vector
from config import TOPICS_CONFIG, EXPERIMENT_CONFIG, DATASET_CONFIG, TOPIC_VECTORS_CONFIG

# Set up logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

def create_samples_from_summaries(tid_positive: int, df: pd.DataFrame) -> List[Tuple[str, str]]:
...
def create_training_samples(topic_encoding_type: str) -> Dict[int, List[Tuple[str, str]]]:
    ...
    if topic_encoding_type == 'zeros':
        ...
        relevant_tids = list(map(int, df['tid1'].unique()))
        for tid in relevant_tids:
            training_samples[tid] = create_samples_from_summaries(tid_positive=tid, df=df)
    else:
...
def train_topic_vectors(topic_encoding_type: str) -> None:
    ...
    for tid, training_samples in training_samples.items():
        ...
        topic_counter += 1
        logger.info(f"Saved topic vector for topic id {tid} to {topic_vector_dir} ({topic_counter}/{num_topics})")
