...
from config import get_generation_config, GENERATION_CONFIG, DATASET_CONFIG, FACTOR_SCALING_DICT, \
CONSTANT_SHIFT_DICT, THRESHOLD_SELECTION_DICT, EXPERIMENT_CONFIG, TOPICS_CONFIG
...
class CustomModel:
    ...
    def generate(self, input_ids: List[int], attention_mask=None, topic_token_ids=None, 
                 scaling_factor: Optional[float] = None, shift_constant: Optional[float] = None,
...
def generate_summaries(dataloader: DataLoader, model: AutoModelForCausalLM, 
    ...
    try:
        ...
        if experiment_name == 'factor_scaling':
            ...
            experiment_information['THRESHOLD_SELECTION_DICT'] = THRESHOLD_SELECTION_DICT
        ...
        with torch.no_grad(): # Disable gradient calculation
            for i, batch in enumerate(dataloader):
                ...
                article, article_idx = batch['article'][0], batch['article_idx'][0].item()
                ...
                topic_tokens = get_topic_tokens(tokenizer=tokenizer, lda=lda, tid=tid1,
                ...
                for factor in factors:
                    ...
                    summary = tokenizer.decode(output.squeeze(), skip_special_tokens=False)
                    article_dict[str(factor)] = summary
        
                generated_summaries.append(article_dict)
        logger.info(f"Generated {len(factors)} summaries, for each of the {num_articles} articles.")
...
def main():
    ...
    if experiment_name not in ['factor_scaling', 'constant_shift', 'threshold_selection']:
    ...
    summary_results = generate_summaries(dataloader=dataloader, model=model, tokenizer=tokenizer,
                                         device=device, lda=lda)

    # Save summaries to a file in the results_logits_reweighting directory
    file_name = (f"{experiment_name}_{model_alias}_{DATASET_CONFIG['num_articles']}_"
                 f"{GENERATION_CONFIG['min_new_tokens']}_{GENERATION_CONFIG['max_new_tokens']}_"
                 f"{GENERATION_CONFIG['num_beams']}.json")
    
    results_dir = os.path.join(parent_dir, 'data', f'results_{experiment_name}', file_name)
