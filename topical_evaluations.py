...
minimum_phi_value = 0.001  # Minimum value of phi to consider when extracting topic words
...
def batch_process_texts(texts: List[str], method: str) -> List[set]:
    ...
    """
    if method == 'lemmatize':
        return [set(token.lemma_ for token in nlp(text.lower()) if token.text not in punctuation and token.text not in stopwords) for text in texts]
    elif method == 'stem':
        return [{stemmer.stem(token) for token in nltk.word_tokenize(text.lower()) if token not in punctuation and token not in stopwords} for text in texts]
    else:
...
def topic_scores(documents: List[str], lda: LdaModel, tids: List[int], method: str, dictionary: Optional[Dict] = None) -> List[float]:
    """
...
def batch_dict_topic_score(documents: List[str], lda: LdaModel, tids: List[int], dictionary: Dict) -> List[float]:
    """
...
def topic_score_multiprocess(documents, lda, tids, method, num_processes=4):
    ...
    for i in range(num_processes):
        ...
        process = Process(target=process_documents_chunk, args=(doc_chunk, lda, tids_chunk, method, result_list))
